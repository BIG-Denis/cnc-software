...
class MainWindow(QMainWindow):
    ...
    def clicked_btn_send_hex(self):
        if not confirm(self, "Отправить HEX пакет?"):
            return
        hex_text = self.lineEdit_pro_hex.text()
        send_hex = hex_text
        # Если галочка включена
        if self.check_auto_crc.isChecked():
            try:
                data = bytes.fromhex(hex_text)
            except ValueError:
                alert(self, "Некорректная HEX-строка. Используйте байты через пробел.")
                return
            if not data:
                alert(self, "HEX-пакет пуст.")
                return
            # Если пакет начинается с 0xAC 0x53,
            # CRC считается начиная с байта LEN (то есть после SYNC); иначе — с первого байта.
            crc_start = 2 if len(data) >= 2 and data[0] == SYNC_1 and data[1] == SYNC_2 else 0
            if len(data) <= crc_start:
                alert(self, "Недостаточно данных для вычисления CRC.")
                return
            if len(data) - crc_start >= 2:
                crc_with_last = Crc8(data[crc_start:-1])
                # Если последний байт уже равен CRC от всех байт кроме последнего — он считается CRC
                # и просто пере‑записывается этим же значением;
                if data[-1] == crc_with_last:
                    data = data[:-1] + bytes([crc_with_last])
                # Иначе — CRC добавляется в конец
                else:
                    data = data + bytes([Crc8(data[crc_start:])])
            else:
                data = data + bytes([Crc8(data[crc_start:])])
            send_hex = data.hex(' ')
        if not SendHex(self.portsComboBox.currentText(), send_hex, int(self.baudRateLineEdit.text())):
            alert(self, "Отправка не удалась. Проверьте соединение и формат пакета.")
            return
        # Иначе просто отправляем
        self.Append(f'<<<< Отправлен HEX-пакет: {send_hex} >>>>\n')
        alert(self, "HEX-пакет отправлен.")
